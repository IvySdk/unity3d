...
//#if UNITY_EDITOR || UNITY_ANDROID
    ...
    public bool HasRewardAd (string tag) {
        ...
        try{
            ...
            Debug.LogError("Check HasRewardAd Catch Exception::::::::"+e.StackTrace);
            return false;
        }
        return false;
    }
    ...
    public delegate void BackHomeAd();
    public BackHomeAd backHomeAd;
    ...
    public bool ShowNativeAd (string tag, int xPixel, int yPixel, int width, int height, int screenWidth, int screenHeight) {
        //BACK_HOME_AD_TIME = GetCurrentTimeInMills ();
#if UNITY_EDITOR
        RiseEditorAd.EditorAdInstance.Toast ("showNativeBanner");
#endif
        if (_class != null) {
            return _class.CallStatic<bool> ("showNativeBanner", tag, xPixel, yPixel, width, height, screenWidth, screenHeight);
        }
        return false;
    }
    ...
    /// <summary>
    /// 判断当前权限状态
    /// 0: 权限被彻底拒绝，需要调整设置页面开启
    /// 1： 权限已开启
    /// 2： 权限状态待定，仍可通过系统接口请求
    /// </summary>
    public int GetNotificationPermissionState()
    {
#if UNITY_EDITOR
        return 1;
#else
        if (_class != null)
        {
           return _class.CallStatic<int>("getNotificationPermissionState");
        }
        return -1;
#endif
    }
    ...
    /// <param name="subtitle">//通知栏内容</param>
    /// <param name="delay">// 延迟时间，单位秒</param>
    /// <param name="autoCancel">// 通知栏是否可关闭</param>
    /// <param name="action"> // 通知栏行为，可为空，默认是打开游戏主界面</param>
    /// <param name="repeat">// 是否为重复任务</param>
    /// <param name="onNetWorkOn">// 是否需要在联网状态显示通知</param>
    /// <param name="requireCharging">// 是否需要在充电状态显示通知</param>
    /// 
    public void PushLocalNotification (string tag, string title, string subtitle, long delay, bool autoCancel=true, string action="", bool repeat=false, bool onNetWorkOn=false,bool requireCharging=false) {
        if (_class != null) {
            Debug.Log("noti push tag " + tag + " delay" + delay);
            _class.CallStatic<bool> ("pushLocalNotification", tag, title, subtitle, delay, autoCancel, action, repeat, onNetWorkOn, requireCharging);
        }
    }
    ...
    public void cancelLocalMessage(string key) {
        if (_class != null) {
            Debug.Log("noti cancel " + key);
            _class.CallStatic("cancelLocalNotification", key);
        }
    }
    ...
    /// 取消远程通知
    /// </summary>
    /// <param name="key">远程通知表示，通知的id</param>
    public void cancelMessage(string key)
    {
        if (_class != null)
        {
            _class.CallStatic("cancelMessage", key);
        }
    }
...
#region GooglePlayCenter
    ...
    private bool adRemoved = false;
    public void EnableRemoveAd (bool removeAd) {
        adRemoved = removeAd;
    }
    ...
    private class RiseEditorAd : MonoBehaviour {
...
#if UNITY_ANDROID
                RiseSdkListener.Instance.onReceiveReward ("0|" + rewardAdId + "|" + rewardAdTag);
#elif UNITY_IOS
        ...
        private IEnumerator CheckToast () {
    ...
    private class FileLRUCache {
        ...
        public FileLRUCache (int capacity) {
            ...
            try {
                ...
                //test
                //RiseSdk.Instance.Toast ("RiseSdk.defFilePath ::: " + defFilePath);
                Debug.Log ("RiseSdk.defFilePath ::: " + defFilePath);
