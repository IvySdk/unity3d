...
public class RiseSdkListener : MonoBehaviour {
    ...
    //#if UNITY_ANDROID
#if !UNITY_IOS
    public static event Action<RiseSdk.NetworkStatus> OnNetworkStatusChanged;
    ...
    public void onDeepLinkReceived(string data) {
        if (OnDeepLinkReceived != null && OnDeepLinkReceived.GetInvocationList().Length > 0) {
        ...
        if (OnAdEvent != null && OnAdEvent.GetInvocationList ().Length > 0) {
            ...
            switch (adType) {
                ...
                case RiseSdk.ADTYPE_VIDEO:
                    eventType = RiseSdk.AdEventType.RewardAdShowStart;
                    break;
    ...
    public void onAdLoaded (string data) {
        if (OnAdEvent != null && OnAdEvent.GetInvocationList ().Length > 0) {
            ...
            switch (adType) {
                case RiseSdk.ADTYPE_INTERTITIAL:
                    eventType = RiseSdk.AdEventType.FullAdLoadCompleted;
                    break;
                case RiseSdk.ADTYPE_VIDEO:
                    eventType = RiseSdk.AdEventType.RewardAdLoadCompleted;
                    break;
                case RiseSdk.ADTYPE_ICON:
                    eventType = RiseSdk.AdEventType.IconAdLoadCompleted;
                    break;
                case RiseSdk.ADTYPE_NATIVE:
                    eventType = RiseSdk.AdEventType.NativeAdLoadCompleted;
                    break;
            }
            //OnAdEvent (RiseSdk.AdEventType.AdClicked, -1, tag, adType);
            OnAdEvent (eventType, -1, tag, adType);
    ...
    public void onNetworkStatusChanged (string data) {
        if (OnNetworkStatusChanged != null && OnNetworkStatusChanged.GetInvocationList ().Length > 0) {
            if (string.IsNullOrEmpty (data)) {
                OnNetworkStatusChanged (RiseSdk.NetworkStatus.Online);
            } else {
                int status = 1;
                int.TryParse (data, out status);
                OnNetworkStatusChanged ((RiseSdk.NetworkStatus) status);
            }
        }
    }
#elif UNITY_IOS
