...
static int64_t _SDKDiskSpaceFree() {
    NSError *error = nil;
...
/// String's md5 hash.
static NSString *_SDKNSStringMD5(NSString *string) {
...
static void _SDKDiskCacheInitGlobal() {
...
static SDKDiskCache *_SDKDiskCacheGetGlobal(NSString *path) {
    if (path.length == 0) return nil;
    _SDKDiskCacheInitGlobal();
...
static void _SDKDiskCacheSetGlobal(SDKDiskCache *cache) {
...
- (void)_trimToFreeDiskSpace:(NSUInteger)targetFreeDiskSpace {
    ...
    int64_t diskFreeBytes = _SDKDiskSpaceFree();
...
- (NSString *)_filenameForKey:(NSString *)key {
    ...
    if (!filename) filename = _SDKNSStringMD5(key);
    return filename;
}
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"SDKDiskCache init error" reason:@"SDKDiskCache must be initialized with a path. Use 'initWithPath:' or 'initWithPath:inlineThreshold:' instead." userInfo:nil];
...
- (instancetype)initWithPath:(NSString *)path
    ...
    SDKDiskCache *globalCache = _SDKDiskCacheGetGlobal(path);
    if (globalCache) return globalCache;
    
    SDKKVStorageType type;
    if (threshold == 0) {
        type = SDKKVStorageTypeFile;
    } else if (threshold == NSUIntegerMax) {
        type = SDKKVStorageTypeSQLite;
    } else {
        type = SDKKVStorageTypeMixed;
    }
    
    SDKKVStorage *kv = [[SDKKVStorage alloc] initWithPath:path type:type];
    if (!kv) return nil;
...
- (id<NSCoding>)objectForKey:(NSString *)key {
    if (!key) return nil;
    Lock();
    SDKKVStorageItem *item = [_kv getItemForKey:key];
    Unlock();
    ...
    if (object && item.extendedData) {
        [SDKDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    NSData *extendedData = [SDKDiskCache getExtendedDataFromObject:object];
    NSData *value = nil;
    ...
    if (!value) return;
    NSString *filename = nil;
    if (_kv.type != SDKKVStorageTypeSQLite) {
