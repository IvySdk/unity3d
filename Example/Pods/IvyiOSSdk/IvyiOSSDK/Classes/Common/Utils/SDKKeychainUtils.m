...
static NSString *SDKKeychainUtilsErrorDomain = @"SDKKeychainUtilsErrorDomain";

#if __IPHONE_OS_VERSION_MIN_REQUIRED < 30000 && TARGET_IPHONE_SIMULATOR

@interface SDKKeychainUtils (PrivateMethods)
(SecKeychainItemRef) getKeychainItemReferenceForUsername: (NSString *) username andServiceName: (NSString *) serviceName error: (NSError **) error;
@end

#endif

@implementation SDKKeychainUtils

#if __IPHONE_OS_VERSION_MIN_REQUIRED < 30000 && TARGET_IPHONE_SIMULATOR

+(NSString *) getPasswordForUsername: (NSString *) username andServiceName: (NSString *) serviceName error: (NSError **) error {
    if (!username || !serviceName) {
        *error = [NSError errorWithDomain: SDKKeychainUtilsErrorDomain code: -2000 userInfo: nil];
        return nil;
    }
    
    SecKeychainItemRef item = [SDKKeychainUtils getKeychainItemReferenceForUsername: username andServiceName: serviceName error: error];
    if (*error || !item) {
        return nil;
    }
    ...
    if (status != noErr) {
        *error = [NSError errorWithDomain: SDKKeychainUtilsErrorDomain code: status userInfo: nil];
        return nil;
    }
...
+ (NSString *) getPasswordForUsername: (NSString *) username andServiceName: (NSString *) serviceName error: (NSError **) error {
    ...
    if (status != noErr) {
        if (status == errSecItemNotFound) {
            ...
            if (error != nil) {
                *error = [NSError errorWithDomain: SDKKeychainUtilsErrorDomain code: -1999 userInfo: nil];
            }
        }
        else {
...

{
    ...
    NSString *existingPassword = [SDKKeychainUtils getPasswordForUsername: username andServiceName: serviceName error:&getError];
