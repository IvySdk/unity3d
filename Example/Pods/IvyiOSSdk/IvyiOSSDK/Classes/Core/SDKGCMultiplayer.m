...
@implementation SDKGCMultiplayer

#pragma mark - Object Lifecycle

+ (SDKGCMultiplayer *)defaultMultiplayerManager {
    static SDKGCMultiplayer *singleton;
...
- (void)beginListeningForMatches {
    ...
    [[SDKGCManager getLocalPlayer] registerListener:self];
}

- (void)dealloc {
    [[SDKGCManager getLocalPlayer] unregisterAllListeners];
}
...
- (void)findMatchWithMinimumPlayers:(int)minPlayers maximumPlayers:(int)maxPlayers onViewController:(UIViewController *)viewController {
    if (![[SDKGCManager sharedManager] isGameCenterAvailable]) {
        ...
        if ([[[SDKGCManager sharedManager] delegate] respondsToSelector:@selector(gc_error:)])
            [[[SDKGCManager sharedManager] delegate] gc_error:error];
...
#else
    #warning SDKGCMultiplayer::sendMatchData not implemented
    return NO;
#endif
}

- (void)disconnectLocalPlayerFromMatch {
    DLog(@"[GC][SDKGCManager] Attempting to disconnect local player");
    
    if (!self.multiplayerMatch) return;
    
    [self.multiplayerMatch disconnect];
    
    DLog(@"[GC][SDKGCManager] Disconnected local player");
...
- (void)matchmakerViewController:(GKMatchmakerViewController *)viewController didFailWithError:(NSError *)error {
    ...
    [self.matchPresentingController dismissViewControllerAnimated:YES completion:nil];
    DLog(@"[GC]Error finding match: %@", error);
}
...
- (void)matchmakerViewController:(GKMatchmakerViewController *)viewController didFindMatch:(GKMatch *)theMatch {
    ...
    if (!self.multiplayerMatchStarted && self.multiplayerMatch.expectedPlayerCount == 0) {
        DLog(@"[GC]Ready to start match!");
        DLog(@"[GC]The didFindMatch: connection is being called. You need to determine if this should be handled.");
...
- (void)match:(GKMatch *)theMatch player:(NSString *)playerID didChangeState:(GKPlayerConnectionState)state {
    ...
    switch (state) {
        case GKPlayerStateConnected:
            ...
            DLog(@"[GC]Player connected!");
            
            if (!self.multiplayerMatchStarted && theMatch.expectedPlayerCount == 0) {
                DLog(@"[GC]Ready to start match!");
                
                // TODO: Match was found and all players are connected
                DLog(@"[GC]The didChangeState: connection is being called. You need to determine if this should be handled. For now it will not be handled.");
...
- (void)match:(GKMatch *)theMatch connectionWithPlayerFailed:(NSString *)playerID withError:(NSError *)error {
    ...
    DLog(@"[GC]Failed to connect to player with error: %@", error);
...
- (void)match:(GKMatch *)theMatch didFailWithError:(NSError *)error {
    ...
    DLog(@"[GC]Match failed with error: %@", error);
