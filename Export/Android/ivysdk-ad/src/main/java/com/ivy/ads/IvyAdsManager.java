...
//      IvySdk.logEvent(EventID.CLICK_SHOW_REWARD_INTERSTITIAL, bundle);
      ...
      if (this.mRewardedInterstitialAds.isLoaded()) {
        ...
        Logger.debug( "Rewarded Interstitial not available. reload");
      }
    } catch(Throwable ex) {
      Logger.error( "showRewardedInterstitial exception");
    }
  }
  ...
  public boolean haveNative() {
    ...
    if (result) {
      Logger.debug( "Native ad already loaded");
      return true;
    }
    ...
    if (!preFillNative) {
      Logger.debug( "preFillNative: false");
      return false;
    }
  ...
  public void onAdShowSuccess(IvyAdInfo adInfo) {
    Logger.debug( String.format("onAdShowSuccess %s, %s", adInfo.getAdType(), adInfo.getAdapter().getName()));
...
//    } catch (Exception e){}
  ...
  public void onAdShowFail(IvyAdType adType) {
    Logger.debug("onAdShowFail::" +  adType);
  ...
  public void fetchNativeAd(Activity activity) {
    Logger.debug( "fetchNativeAd start");
    if (this.mNativeAds != null) {
