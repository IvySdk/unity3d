...
public enum FirebaseAuthError {
    ...
    public static FirebaseAuthError fromException(Exception paramException) {
        try {
            if (paramException instanceof FirebaseAuthException)
                return valueOf(((FirebaseAuthException) paramException).getErrorCode());
            if (paramException instanceof com.google.firebase.FirebaseNetworkException)
                return ERROR_NETWORK;
            if (paramException instanceof com.google.firebase.FirebaseTooManyRequestsException)
                return ERROR_TOO_MANY_REQUESTS;
            if (paramException instanceof com.google.firebase.FirebaseApiNotAvailableException)
                return ERROR_API_NOT_AVAILABLE;
            Logger.error("FirebaseAuthError", "exception: " + paramException.getClass().getName(), paramException);
    ...
    FirebaseAuthError(String paramString1) {
