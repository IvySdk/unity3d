...
public final class IvySdk {
    ...
    private static com.ivy.player.Player curPlayer = null;

    public static void setPlayer(String data) {
        curPlayer = com.ivy.player.Player.build(data);
    }

    public static com.ivy.player.Player getPlayer() {
        return curPlayer;
    }
    ...
    public static synchronized void initialize(@NonNull final Activity activity, final Bundle savedInstanceState, final InitializeCallback callback) {
        ...
        AdjustConfig config = new AdjustConfig(activity, appToken, BuildConfig.DEBUG ? "sandbox" : "production");
        ...
        config.setOnInstallReferrerReadListener(new InstallReferrerReadListener() {
            @Override
            public void onInstallReferrerRead(String installReferrer, long referrerClickTimestampSeconds, long installBeginTimestampSeconds) {
                Log.w(TAG, "onInstallReferrerRead, installReferrer: " + installReferrer);
                if (installReferrer != null && !"".equals(installReferrer)) {
                    sp.edit().putString("parfka_install_referrer", installReferrer).apply();
                    try {
                        if (eventTracker == null) {
                            new Handler().postDelayed(new Runnable() {
                                @Override
                                public void run() {
                                    Log.w(TAG, "onInstallReferrerRead, after 10s: " + installReferrer);
                                    Bundle bundle = new Bundle();
                                    bundle.putString("referrer", installReferrer);
                                    bundle.putLong("referrerClick", referrerClickTimestampSeconds);
                                    bundle.putLong("installBegin", installBeginTimestampSeconds);
                                    eventTracker.parfkaLog("installReferrer", bundle);
                                }
                            }, 10 * 1000);
                        } else {
                            Log.w(TAG, "onInstallReferrerRead, immiter: " + installReferrer);
                            Bundle bundle = new Bundle();
                            bundle.putString("referrer", installReferrer);
                            bundle.putLong("referrerClick", referrerClickTimestampSeconds);
                            bundle.putLong("installBegin", installBeginTimestampSeconds);
        ...
        String parfkaUrl = sp.getString(SP_KEY_PARFKA_URL, null);
        String parfkaUrl2 = sp.getString(SP_KEY_PARFKA_URL2, null);

        if (parfkaUrl != null && !"".equals(parfkaUrl)) {
            AdjustFactory.setBaseUrl(parfkaUrl);
        } else {
            if (ai != null && ai.metaData != null) {
                Object baseUrlO = ai.metaData.get("parfka.baseUrl");
                if (baseUrlO instanceof String) {
                    String bUrl = String.valueOf(baseUrlO);
                    if (!"".equals(bUrl)) {
                        AdjustFactory.setBaseUrl(bUrl);
                    }
                }
            }
        }
        ...
        if (gridData != null && gridData.has("parfkaUrl")) {
            String newParfkaUrl = gridData.optString("parfkaUrl");
            if (newParfkaUrl != null && !"".equals(newParfkaUrl)) {
                AdjustFactory.setBaseUrl(newParfkaUrl);
                sp.edit().putString(SP_KEY_PARFKA_URL, newParfkaUrl).apply();

                parfkaUrl = newParfkaUrl;
            }
        }

        if (gridData != null && gridData.has("parfkaUrl.2")) {
            String newParfkaUrl2 = gridData.optString("parfkaUrl.2");
            if (newParfkaUrl2 != null && !"".equals(newParfkaUrl2)) {
                sp.edit().putString(SP_KEY_PARFKA_URL2, newParfkaUrl2).apply();
                parfkaUrl2 = newParfkaUrl2;
            }
        }

        adEnabled = sp.getBoolean("adsfall_ad_status", true);

            DLog.d("current is google billing support");
            // 内购
        ...
        initRemoteConfig(debugMode, gridData, callback);
        ...
        // check should change the base url
        checkShouldChangeParfkaUrl(parfkaUrl, parfkaUrl2);
        ...
        if (gridData.has("appflyers.devkey")) {
            ...
            if (!"".equals(appflyersDevKey)) {
                ...
                if (gridData != null && gridData.has("af.invite.template.id")) {
                    String templateId = gridData.optString("af.invite.template.id", "");
                    if (!TextUtils.isEmpty(templateId)) {
                        Log.d("ADSFALL", "Deeplink set one link mode:" + templateId);
                        AppsFlyerLib.getInstance().setAppInviteOneLink(templateId);
                    }
                }
                if (debugMode) {
                ...
                AppsFlyerLib.getInstance().subscribeForDeepLink(new DeepLinkListener() {
                    ...
                    public void onDeepLinking(@NonNull DeepLinkResult deepLinkResult) {
                        Log.d("ADSFALL", "Deeplink call back");
                            ...
                            if (!TextUtils.isEmpty(inviteUserId)) {
                                ...
                                if (inviteUserId.equals(currentUserId)){
                                    ...
                                    if (dlData.has("deep_link_sub2")){
                                        String inviteAppId = deepLinkObj.getStringValue("deep_link_sub2");
                                        inviteUserId = inviteUserId + "|" + inviteAppId;
                                    }
                ...
                AppsFlyerConversionListener conversionListener = new AppsFlyerConversionListener() {
                    ...
                    public void onConversionDataSuccess(Map<String, Object> conversionData) {
                        ...
                        Log.e("ADSFALL", "appsflyer open conversion:" + (conversionData == null ? "" : conversionData.toString()));
...
//                purchaseClient.startObservingTransactions();
        ...
        cmpUtil.init(activity, debugMode, new ICMPCallback() {
            @Override
            public void onResult(boolean ableLoadAds, int consentStatus) {
                ...
                try {
                    ...
                    Log.e("CMP", "setup consent status :" + status);

                    if (AdmobManager.isCalledInitialized) {
                        return;
                    }
                    ...
                    if (eventTracker != null) {
                        ...
                        eventTracker.logToFirebase("admob_sdk_init_start", bundle);
                    }
                    Log.e("admobLog", "call Admob init method");
                    AdmobManager.getInstance().initialize(activity, initializationStatus -> {
                        if (eventTracker != null) {
                            Bundle bundle = new Bundle();
                            bundle.putString("flow_seq", "02");
                            eventTracker.logToFirebase("admob_sdk_init_complete", bundle);
                        }
                        try {
                            Logger.debug(TAG, "Admob initialized");
                            Log.e("admobLog", "Admob initialized");
                            if (finalDebugMode) {
                            ...
                            for (Map.Entry<String, AdapterStatus> entry : adapterStatusMap.entrySet()) {
                                ...
                                Log.e("admobLog", "adapter status: " + entry.getKey() + " >>> " + entry.getValue().getInitializationState());
                            }
    ...
    private static void initRemoteConfig(boolean debugMode, JSONObject gridData, InitializeCallback callback) {
        try {
...
//      IronSource.onResume(activity);
    ...
    private static final String TAG_SPLASH_BANNER_VIEW = "splash_banner_view";
    ...
    public static void showSplashBannerAd(int adPos) {
        if (!adEnabled) {
            return;
        }
        final Activity activity = getActivity();
        ...
        if (!(adPos == POS_CENTER_BOTTOM || adPos == POS_CENTER_TOP)) {
            return;
        }
        activity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                ...
                View bannerView = rootView.findViewWithTag(TAG_SPLASH_BANNER_VIEW);
                if (bannerView != null) {
                ...
                try {
                    ...
                    rl.setTag(TAG_SPLASH_BANNER_VIEW);
                    rootView.addView(rl, lp);

                    adManager.showSplashBanner(rl);
...
//        openPlaystoreInBrowser(activity);
        ...
        try {
            // 由于份额问题，只显示3次
            final int[] lastInApp = {mmGetIntValue("_last_in_app_rate_", 0)};
            if (lastInApp[0] < 3) {
                ReviewManager reviewManager = ReviewManagerFactory.create(activity.getApplicationContext());
                ...
                request.addOnCompleteListener(new OnCompleteListener<ReviewInfo>() {
                    ...
                    public void onComplete(@NonNull Task<ReviewInfo> task) {
                        if (task.isSuccessful()) {
                            lastInApp[0] += 1;
                            mmSetIntValue("_last_in_app_rate_", lastInApp[0]);
    ...
    public static PurchaseManager getPurchaseManager(@NonNull Context context) {
        if (IvySdk.getGridConfigBoolean("payermax")) {
            return new PayerMaxPurchaseImpl(context, EventBus.getInstance(), eventTracker);
        }
    ...
    public static void logToFirebase(String eventName, Bundle bundle) {
        Log.e("firebase-event", "pre::" + eventName + "::" + bundle.toString());
        if (eventTracker != null) {
            Log.e("firebase-event", "in::" + eventName + "::" + bundle.toString());
    ...
    public static double getGridConfigDouble(String key, double defaultValue) {
        if (GridManager.getGridData() == null) {
            return defaultValue;
        }
        return GridManager.getGridData().optDouble(key, defaultValue);
    }
    ...
    /**
     * iterator all urls and try to fetch the reachable one,
     * if none of the urls are available, set the fist one
     *
     * @param url1 base url
     * @param url2 backup url
     * @return the reachable url
     */
    private static void checkShouldChangeParfkaUrl(final String url1, final String url2) {
        if (url1 == null || url2 == null || "".equals(url1) || "".equals(url2)) {
            Log.w(TAG, "No event url defined");
            return;
        }
        ...
        Logger.debug(TAG, "Master URL: " + url1);
        Logger.debug(TAG, "Fallback URL: " + url2);
        // iterator all urls and try to pick the reachable one
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                for (String url : urls) {
                    ...
                    try {
                        response = getOkHttpClient().newCall(new Request.Builder().url(url + "/event").build()).execute();
                        if (response != null && response.code() == 200) {
                            Logger.debug(TAG, "URL: " + url + " is ok");
                            selectedUrl = url;
                            break;
                        } else {
                            Logger.debug(TAG, "URL: " + url + " is unreachable");
                        }
                    } catch (Throwable t) {
                        Logger.error(TAG, "URL unreachable" + url, t);
                    } finally {
                        if (response != null) {
                            try {
                                response.close();
                ...
                if (selectedUrl != null && !selectedUrl.equals(url1)) {
                    Logger.error(TAG, "The master url is unreachable, we picked a new one to: " + selectedUrl);
                    AdjustFactory.setBaseUrl(selectedUrl);
...
//        }
//        return true;
        return NotificationPermissionUtil.isPermissionEnabled(activity);
    }

    public static int getNotificationPermissionState(Activity activity){
        if (activity == null || activity.isFinishing()) {
            Logger.warning(TAG, "activity not valid, requestNotificationPermission ignore");
            return 1;
        }
        return NotificationPermissionUtil.getPermissionState(activity);
    }
    ...
    public static void requestNotificationPermission(Activity activity){
        ...
        NotificationPermissionUtil.requestPermission(activity);
    }
    ...
    public static void openNotificationSettings(Activity activity) {
        ...
        NotificationPermissionUtil.openPermissionSetting(activity);
//        try {
...
//        }
    ...
    public static void mmSetDoubleValue(String key, double value) {
        if (gameDataMMKV == null) {
            Log.e(TAG, "gameDataMMKV invalid");
            return;
        }
    ...
    public static double mmGetDoubleValue(String key, double defaultValue) {
        if (gameDataMMKV == null) {
            Log.e(TAG, "gameDataMMKV invalid");
            return defaultValue;
        }
        return gameDataMMKV.decodeDouble(key, defaultValue);
    }
    ...
    public static boolean pushLocalNotification(String tag,
        ...
        try {
            Activity activity = getActivity();
            if (activity == null) {
                Logger.error("error to push local notification, activity is null");
                return false;
            }
            if (!NotificationPermissionUtil.isPermissionEnabled(activity)) {
                Logger.error("error to push local notification, no Permission");
    ...
    public static void cancelLocalNotification(String key){
        try {
            if (TextUtils.isEmpty(key)) {
                NotificationMan.cancelAll(getActivity().getApplicationContext());
            } else {
                NotificationMan.cancelTask(getActivity().getApplicationContext(), key);
            }
        } catch (Exception e){
            e.printStackTrace();
        }
