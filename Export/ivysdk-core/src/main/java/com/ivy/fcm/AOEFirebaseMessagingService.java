...
import com.ivy.notification.NotificationUtil;
    ...
    try {
      ...
      if (data != null && data.size() > 0) {
        ...
        if (data.containsKey("notification")) {
          String notificationData = data.get("notification");
          try{
            NotificationUtil.showFirebaseRemoteNotification(getApplicationContext(), notificationData);
          } catch (Exception e){
            Logger.error(TAG, "parse remote notification msg err:" + e.getMessage());
          }
        }
      } else if (remoteMessage.getNotification() != null) {
        //推送通知栏，这种情况 fcm 会生成默认的通知消息，无须处理
      }
    } catch (Throwable t) {
  ...
  private void sendRegistrationToServer(String token) {
    ...
    try {
      if (token != null && IvySdk.getActivity() != null) {
        Adjust.setPushToken(token, IvySdk.getActivity());
      }
    } catch (Exception ex) {
      ex.printStackTrace();
    }
