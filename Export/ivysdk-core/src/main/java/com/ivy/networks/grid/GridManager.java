...
public final class GridManager {
    ...
    private static Map<String, List<PAM>> pamMap = new HashMap<>();
    ...
    public GridManager(Activity activity, EventTracker eventTracker, long startupTime, boolean forceUpdate) {
        ...
        try {
            ...
            try {
                String pam = gridData.getString("pam_2_0");
                setupPAM(pam);
            } catch (Exception e) {
                Logger.error("get pam config from default.json failed!!!" + e.getMessage());
            }
        }
    ...
    private synchronized boolean downloadGridData(Activity activity) {
        ...
        try {
            ...
            Log.e("ADSFALL", "get pam config from default.json failed!!!" + e.getMessage());
        }
    ...
    private void mergeGridWithRemoteConfig() {
        ...
        //PAM 2.0
        String pamConfig = firebaseRemoteConfig.getString("pam_2_0");
        try {
            Log.e("ADSFALL", "pam remote::" + pamConfig);
            if (!TextUtils.isEmpty(pamConfig)) {
                gridData.put("pam_2_0", pamConfig);
                setupPAM(pamConfig);
            }
        } catch (Exception e) {
            Log.e("ADSFALL", "pam remote error::" + e.getMessage());
            e.printStackTrace();
        }
    }

    private void setupPAM(String data) {
        Log.e("ADSFALL", "setupPAM::" + data);
        try {
            Map<String, List<PAM>> map = new HashMap<>();
            String[] pamType = new String[]{"rewarded", "interstitial", "banner", "rewarded_interstitial"};
            JSONObject root = new JSONObject(data);
            for (String type : pamType) {
                JSONObject item = root.optJSONObject(type);
                if (item != null) {
                    JSONArray userGroup = item.optJSONArray("userGroup");
                    if (userGroup != null) {
                        int count = userGroup.length();
                        List<PAM> list = new ArrayList<>();
                        for (int i = 0; i < count; i++) {
                            PAM pam = PAM.format(userGroup.optJSONObject(i));
                            if (pam != null) {
                                list.add(pam);
                            }
                        }
            ...
            Log.e("ADSFALL", "setupPAM::error::" + e.getMessage());
            e.printStackTrace();
        } finally {
            Log.e("ADSFALL", "setupPAM::result::" + pamMap.size());
        }
    }

    private static float total_memory = -1;

    public static PAM getPAM(IvyAdType adType, double lastValue) {
        Log.e("ADSFALL", "getPAM::" + adType.name() + "::" + lastValue);
        List<PAM> list = null;
        PAM base = null;
        if (total_memory == -1) {
            total_memory = AndroidSdk.getTotalMem() / 1024.f / 1024.f;
        }
        try {
            if (adType == IvyAdType.INTERSTITIAL) {
                list = pamMap.get("interstitial");
            } else if (adType == IvyAdType.REWARDED) {
                list = pamMap.get("rewarded");
            } else if (adType == IvyAdType.BANNER) {
                list = pamMap.get("banner");
            } else if (adType == IvyAdType.REWARDED_INTERSTITIAL) {
                list = pamMap.get("rewarded_interstitial");
            }
            if (list != null) {
                Iterator<PAM> it = list.iterator();
                while (it.hasNext()) {
                    PAM pam = it.next();
                    if (total_memory >= pam.minRam && total_memory < pam.maxRam && lastValue >= pam.min && lastValue < pam.max) {
                        Log.e("ADSFALL", "getPAM::" + adType.name() + "::find::" + pam);
                        return pam;
                    } else if (pam.min == pam.max && pam.min == -1) {
                        base = pam;
                    }
                }
            }
        ...
        Log.e("ADSFALL", "getPAM::" + adType.name() + "::find::" + base);
        return base;
    }
