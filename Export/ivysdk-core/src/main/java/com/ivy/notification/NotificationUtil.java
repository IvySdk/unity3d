...
public class NotificationUtil {
    ...
    public static void showNotification(Context context,
        ...
        NotificationUtil.showNotification(context, CHANNEL_ID, CHANNEL_NAME, title, subtitle, bigText, largeIcon, bigPicture, autoCancel, action);
    }
        ...
        try {
            ...
            try {
                if (!TextUtils.isEmpty(largeIcon)) {
                    int resourceId = IvySdk.getActivity().getResources().getIdentifier(largeIcon, "drawable", IvySdk.getActivity().getPackageName());
                    if (resourceId != 0) {
                        bitmap_large_icon = BitmapFactory.decodeResource(IvySdk.getActivity().getResources(), resourceId);
            ...
            if (!TextUtils.isEmpty(bigPicture)) {
                int resourceId = IvySdk.getActivity().getResources().getIdentifier(bigPicture, "drawable", IvySdk.getActivity().getPackageName());
                if (resourceId != 0) {
                    Bitmap bitmap_big_picture = BitmapFactory.decodeResource(IvySdk.getActivity().getResources(), resourceId);
