...
import com.parfka.adjust.sdk.scheduler.ThreadScheduler;
...
public class PackageHandler implements IPackageHandler {
  ...
  private static List<ActivityPackage> packageQueue;
  private static AtomicBoolean isSending;
  ...
  private static final int MAX_PACKAGE_QUEUE_SIZE = 1000;
  ...
  @Override
  public void sendNow(final ActivityPackage activityPackage) {
    scheduler.submit(new Runnable() {
      @Override
      public void run() {
        requestHandler.sendNow(activityPackage);
      }
    });
  }
  ...
  private void addI(ActivityPackage newPackage) {
    ...
    if (packageQueue.size() > MAX_PACKAGE_QUEUE_SIZE) {
      logger.warn("max package queue size more than 3000, discard the first");
  ...
  public void updatePackagesI(SessionParameters sessionParameters) {
    ...
    for (ActivityPackage activityPackage : packageQueue) {
      Map<String, Object> parameters = activityPackage.getParameters();

      // partner parameters
      Map<String, Object> mergedPartnerParameters = Util.mergeParameters(sessionParameters.partnerParameters,
        activityPackage.getPartnerParameters(),
        "Partner");

      PackageBuilder.addJsonAll(parameters, mergedPartnerParameters);
  ...
  private void writePackageQueueI() {
    ...
    logger.debug("Package handler wrote %d packages, cost:  %d", packageQueue.size(), (System.currentTimeMillis() - start));
  }
