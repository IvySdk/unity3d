...
/**
 SDKKVStorageItem is used by `SDKKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface SDKKVStorageItem : NSObject
...
/**
 Storage type, indicated where the `SDKKVStorageItem.value` stored.
 ...
 * If you want to store large number of small datas (such as contacts cache), 
   use SDKKVStorageTypeSQLite to get better performance.
 * If you want to store large files (such as image cache),
   use SDKKVStorageTypeFile to get better performance.
 * You can use SDKKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, SDKKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    SDKKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    SDKKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    SDKKVStorageTypeMixed = 2,
};



/**
 SDKKVStorage is a key-value storage based on sqlite and file system.
 Typically, you should not use this class directly.
 
 @discussion The designated initializer for SDKKVStorage is `initWithPath:type:`. 
 ...
 */
@interface SDKKVStorage : NSObject
...
@property (nonatomic, readonly) SDKKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(SDKKVStorageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 If the `type` is SDKKVStorageTypeFile, then the item.filename should not be empty.
 If the `type` is SDKKVStorageTypeSQLite, then the item.filename will be ignored.
 It the `type` is SDKKVStorageTypeMixed, then the item.value will be saved to file 
...
/**
 ...
 SDKKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is SDKKVStorageTypeFile, then the `filename` should not be empty.
 If the `type` is SDKKVStorageTypeSQLite, then the `filename` will be ignored.
 It the `type` is SDKKVStorageTypeMixed, then the `value` will be saved to file
 ...
 */
- (nullable SDKKVStorageItem *)getItemForKey:(NSString *)key;
 ...
 */
- (nullable SDKKVStorageItem *)getItemInfoForKey:(NSString *)key;
...
/**
 ...
 @param keys  An array of specified keys.
 @return An array of `SDKKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<SDKKVStorageItem *> *)getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `SDKKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<SDKKVStorageItem *> *)getItemInfoForKeys:(NSArray<NSString *> *)keys;
