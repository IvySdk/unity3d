...
/**
 SDKMemoryCache is a fast in-memory cache that stores key-value pairs.
 ...
 SDKMemoryCache objects differ from NSCache in a few ways:
 ...
 * It can be configured to automatically evict objects when receive memory 
 ...
 The time of `Access Methods` in SDKMemoryCache is typically in constant time (O(1)).
 */
@interface SDKMemoryCache : NSObject
 ...
 */
@property (nullable, copy) void(^didReceiveMemoryWarningBlock)(SDKMemoryCache *cache);

/**
 A block to be executed when the app enter background.
 The default value is nil.
 */
@property (nullable, copy) void(^didEnterBackgroundBlock)(SDKMemoryCache *cache);
