...
@implementation SDKGCDTimerStamp

@end

@interface SDKGCDTimer2()
...
{
    SDKGCDTimer2 *timer = [[SDKGCDTimer2 alloc] initWithDuration:duration interval:interval isCountDown:isCountDown progressBlock:onUpdate completionBlock:onComplete];
    [timer resume];
    return timer;
}

- (instancetype)initWithDuration:(NSTimeInterval)duration interval:(NSInteger)interval isCountDown:(BOOL)isCountDown progressBlock: (SDKGCDTimerProgressBlock)progressBlock completionBlock: (SDKGCDTimerCompletion)completion{
    if (self = [super init]) {
        self.isCountDownTimer = isCountDown;
...
- (void)setInterval:(NSInteger)interval{
    _interval = interval;
    self.onceTime = [self value: (NSTimeInterval)interval byOpration: OMDecimalOprationTypeDivide percision: 6 withValue: 1000.0].doubleValue;
}
    ...
    if (!self.isCountDownTimer) {
        ...
        if (theNewValue > self.endTime) {
            if (self.progressBlock) {
                SDKGCDTimerStamp *progress = [self timeProgress: theNewValue];
...
/** 计算时间进度*/
- (SDKGCDTimerStamp *)timeProgress: (NSTimeInterval)time{
    ...
    SDKGCDTimerStamp *progress = [[SDKGCDTimerStamp alloc] init];
...
- (void)configure{
    ...
    self.currentTime = self.isCountDownTimer ? (self.endTime) : (0.00);
    [self initTimerCompletion:^{
        if ((self.interval < 10) || (self.interval > 1000)) {
            @throw [NSException exceptionWithName: @"SDKGCDTimer-精度异常" reason: @"precision参数配置异常，大于1000毫秒的计时回调将无意义，以及小于10毫秒无实际意义" userInfo: nil];
        }
        if (self.isCountDownTimer && self.duration <= 0) {
            @throw [NSException exceptionWithName: @"SDKGCDTimer-参数：timerInterval配置失败" reason:@"倒计时模式下开始时时间不能为小于或等于0" userInfo: nil];
        }
        SDKGCDTimerStamp *progress = [self timeProgress: self.currentTime];
        if (!self.isCountDownTimer) {
...
- (void)initTimerCompletion: (void(^)(void))completion{
    ...
    /** 设置计时器*/
    dispatch_source_set_timer(timer, dispatch_walltime(nil, 0), self.interval*NSEC_PER_MSEC, 0);
    /** 设置计时器的里操作事件*/
...
- (void)dealloc{
    ...
    [self stop];
    NSLog(@"❤️THE SDKGCDTimer WAS DEALLOC!");
}
