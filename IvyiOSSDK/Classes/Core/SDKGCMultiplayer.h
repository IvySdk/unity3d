...
@protocol SDKGCMultiplayerDelegate;

@interface SDKGCMultiplayer : NSObject <GKMatchmakerViewControllerDelegate, GKMatchDelegate, GKLocalPlayerListener>

/// Returns the default instance of the multiplayer manager
+ (SDKGCMultiplayer *)defaultMultiplayerManager;

/// SDKGCManager multiplayerDelegate property that should be used to set the delegate for multiplayer matches
@property (nonatomic, weak) id <SDKGCMultiplayerDelegate> multiplayerDelegate;

/// Begin listening and responding to Game Center match requests / invites. This should be one of the first calls made to the SDKGCMultiplayer object.
...
/// SDKGCManager Multiplayer Delegate. Handles multiplayer connections, match data, and other aspects of live multiplayer matchs.
@protocol SDKGCMultiplayerDelegate <NSObject>


@required

/// Sent to the delegate when a live multiplayer match begins
- (void)gameCenterManager:(SDKGCMultiplayer *)manager matchStarted:(GKMatch *)match;

/// Sent to the delegate when a live multiplayer match ends
- (void)gameCenterManager:(SDKGCMultiplayer *)manager matchEnded:(GKMatch *)match;

/// Sent to the delegate when data is recieved on the current device (sent from another player in the match)
- (void)gameCenterManager:(SDKGCMultiplayer *)manager match:(GKMatch *)match didReceiveData:(NSData *)data fromPlayer:(NSString *)playerID;


@optional

/// Sent to the delegate when all players are connected. Passes an array of players in the match to the delegate.
- (void)gameCenterManager:(SDKGCMultiplayer *)manager match:(GKMatch *)match didConnectAllPlayers:(NSArray *)gkPlayerArray;

/// Sent to the delegate when a player is disconnected. Passes the disconnected player to the delegate.
- (void)gameCenterManager:(SDKGCMultiplayer *)manager match:(GKMatch *)match playerDidDisconnect:(GKPlayer *)player __OSX_AVAILABLE_STARTING(__OSX_10_9,__IPHONE_7_0);
...
- (void)gameCenterManager:(SDKGCMultiplayer *)manager match:(GKMatch *)match didRecieveMatchInvitationForPlayer:(GKPlayer *)invitedPlayer playersToInvite:(NSArray *)players;

/// Sent to the delegate when a player accepts a match invitation. Use this opportunity to handle your match.
- (void)gameCenterManager:(SDKGCMultiplayer *)manager match:(GKMatch *)match didAcceptMatchInvitation:(GKInvite *)invite player:(GKPlayer *)player;
